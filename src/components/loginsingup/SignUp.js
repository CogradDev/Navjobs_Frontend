			...
			<div className={`max-w-md w-full space-y-8 shadow-lg rounded-xl p-4 pb-8`}>
				...
				<form className="mt-8 space-y-6" onSubmit={(e) => e.preventDefault()}>
					<div className={`space-y-2`}>
						<div className="flex items-center justify-center space-x-4">
							<div className="flex items-center mr-4">
								...
								/>
								<label htmlFor="applicant" className="ml-2 text-sm text-gray-700 cursor-pointer">
									Applicant
								</label>
							</div>
							<div className="flex items-center">
								<input
									...
									className="form-radio h-4 w-4 text-blue-700"
								/>
								<label htmlFor="recruiter" className="ml-2 text-sm text-gray-700 cursor-pointer">
									Recruiter
								</label>
							</div>
						</div>
							...
							<div className="space-y-2">
								<div>
									...
									<input
										...
										name="resume"
										type="file"
										accept=".pdf,.doc,.docx" // Add accepted file types
										required
										onChange={(event) => handleChange('resume', event.target.files[0])} // Update to handle file change
									/>
								</div>
								<div>
						...
						<div>
							...
							<input
								...
								disabled={!isOTPSend}
								className={`appearance-none rounded-md relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm ${
									isOTPSend ? 'cursor-auto' : 'cursor-not-allowed'
								}`}
					...
					<div>
						...
						>
							{isOTPSend ? (
								<span>Verify OTP and Create Account</span>
							) : (
								<span>Send OTP on Email</span>
							)}
						</button>
					</div>
				</form>
